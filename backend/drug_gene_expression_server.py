...
# Add parent directory to path to import predict_all_genes functions
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

try:
    from standalone_drug_descriptors import get_drug_descriptors
    from predict_all_genes import ImprovedMLP, get_all_training_genes, predict_gene_expression
    DRUG_PREDICTION_AVAILABLE = True
except ImportError as e:
    print(f"Warning: Drug prediction modules not available: {e}")
...
def get_go_terms(gene_list, species='human'):
    ...
    go_terms = {
        ...
        'GO:0007165': {'name': 'signal_transduction', 'genes': gene_list[:len(gene_list)//5]},
        'GO:0006954': {'name': 'inflammatory_response', 'genes': gene_list[len(gene_list)//5:2*len(gene_list)//5]},
        'GO:0006468': {'name': 'protein_phosphorylation', 'genes': gene_list[2*len(gene_list)//5:3*len(gene_list)//5]},
        'GO:0006508': {'name': 'proteolysis', 'genes': gene_list[3*len(gene_list)//5:4*len(gene_list)//5]},
        'GO:0006351': {'name': 'transcription_dna_templated', 'genes': gene_list[4*len(gene_list)//5:]},
    }
    return go_terms
...
def create_volcano_plot(data):
    ...
    # Update layout
    fig.update_layout(
        title='Volcano Plot - Drug-Induced Gene Expression',
...
def create_go_enrichment_plot(enrichment_results, top_n=20):
    ...
    fig.update_layout(
        title='GO Enrichment Analysis - Drug Response',
        xaxis_title='-log10(P-value)',
        yaxis_title='GO Terms',
        width=800,
        height=600,
...
def generate_p_values_from_predictions(predictions):
    """
    Generate mock p-values for gene expression predictions
    In a real scenario, these would come from statistical tests
    """
    # For demonstration, we'll generate p-values based on the magnitude of log2fc
    # Higher absolute log2fc values get lower p-values
    p_values = []
    
    for pred in predictions:
        log2fc = abs(pred['log2fc'])
        
        # Generate p-value based on effect size
        if log2fc > 2.0:
            p_val = np.random.uniform(0.001, 0.01)  # Very significant
        elif log2fc > 1.0:
            p_val = np.random.uniform(0.01, 0.05)   # Significant
        elif log2fc > 0.5:
            p_val = np.random.uniform(0.05, 0.1)    # Marginally significant
        else:
            p_val = np.random.uniform(0.1, 0.5)     # Not significant
...
def predict_drug_gene_expression(drug_name, model_path="best_fast_model.pth"):
    ...
    if result is None:
        raise ValueError(f"Could not find drug: {drug_name}")
    
    qsar_features = np.array(result[4:], dtype=np.float64)
    print(f"QSAR features: {len(qsar_features)} values")
    
    # Get all genes from training data
    print("Extracting genes from training data...")
    gene_list = get_all_training_genes(model_path)
    print(f"Found {len(gene_list)} genes to predict")
    ...
    for i, gene in enumerate(gene_list):
        try:
            pred = predict_gene_expression(qsar_features, gene, model_path)
            predictions.append({'gene': gene, 'log2fc': pred})
            
            if (i + 1) % 50 == 0:
                print(f"  Processed {i + 1}/{len(gene_list)} genes...")
                
        except Exception as e:
            print(f"  Error predicting {gene}: {e}")
            predictions.append({'gene': gene, 'log2fc': 0.0})
    
    # Generate p-values for the predictions
    p_values = generate_p_values_from_predictions(predictions)
    
    # Combine predictions with p-values
    gene_expression_data = []
    for pred, p_val in zip(predictions, p_values):
    ...
    return gene_expression_data, result[0]  # Return drug name as well

@app.route("/api/analyze_drug", methods=['POST'])
def analyze_drug():
    """
    Main endpoint for drug-based gene expression analysis
    """
    if not DRUG_PREDICTION_AVAILABLE:
        return jsonify({'error': 'Drug prediction modules not available. Please ensure predict_all_genes.py and standalone_drug_descriptors.py are accessible.'}), 500
...
def analyze_expression():
    """
    Original endpoint for direct gene expression data analysis
    """
    try:
        data = request.get_json()
...
def index():
    """
    Updated HTML page for testing the drug analysis API
    """
    html = """
    <!DOCTYPE html>
    <html>
    <head>
        <title>Drug Gene Expression Analysis API</title>
        ...
        <style>
            ...
            .drug-input { display: flex; align-items: center; margin: 10px 0; }
    ...
    <body>
        <div class="container">
            <h1>🧬 Drug Gene Expression Analysis API</h1>
            <div class="info">
                <h3>Available Endpoints:</h3>
                <ul>
                    <li><strong>POST /api/analyze_drug</strong> - Analyze drug effects on gene expression (NEW!)</li>
                    <li><strong>POST /api/analyze_expression</strong> - Direct gene expression analysis</li>
            ...
            <div class="info">
                ...
                <div class="drug-input">
                    ...
                    <input type="text" id="drugName" placeholder="e.g., aspirin" value="aspirin">
                    <button onclick="analyzeDrug()">Analyze Drug</button>
                </div>
                <p><small>Enter a drug name to predict its effects on gene expression</small></p>
            </div>
            
            <div class="info">
                <h3>📊 Direct Gene Expression Analysis</h3>
        ...
        <script>
            ...
            async function analyzeDrug() {
                ...
                statusDiv.innerHTML = '<div class="loading">🔬 Analyzing drug: ' + drugName + '...</div>';
                ...
                try {
                    ...
                    document.getElementById('results').innerHTML = `
                        <div class="info">
                            <h3>🎯 Drug Analysis Results for: ${result.drug_name}</h3>
                            <p>Total genes analyzed: ${result.total_genes_count}</p>
                            <p>Significantly affected genes: ${result.significant_genes_count}</p>
